...
var _ = Describe("McoRPC/Golang/RPCUtil", func() {
	...
	BeforeEach(func() {
		...
		ti, err = StartBrokerAndServer(cfg, wg)
		Expect(err).ToNot(HaveOccurred())
		ctx = ti.Ctx

		fw = ti.Choria
		am = agents.New(requests, fw, nil, ti.ChoriaServer, logrus.WithFields(logrus.Fields{"test": "1"}))
		rpcutil, err = New(am)
		Expect(err).ToNot(HaveOccurred())
	...
	var _ = Describe("inventoryAction", func() {
		It("Should retrieve the correct info", func() {
			build.Version = "1.0.0"
			err = ti.ChoriaServer.RegisterAgent(ctx, "rpcutil", rpcutil)
			Expect(err).ToNot(HaveOccurred())

			cfg.Collectives = []string{"mcollective", "other"}
			cfg.MainCollective = "mcollective"

			rpcutil.SetServerInfo(ti.ChoriaServer)
